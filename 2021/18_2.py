...
def test_magnitude():
    ...
    s = SnailFishNumber.from_str('[[2,[[7,7],7]],[[5,8],[[9,3],[0,2]]]]') + \
...
print('\n'.join([str(_) for _ in terms]))
max_sum_mag = 0
for possible_sums in itertools.permutations(terms, 2):
    sum = possible_sums[0] + possible_sums[1]
    sum_mag = sum.magnitude
    if sum_mag > max_sum_mag:
        print(f"New max: {possible_sums[0]} + {possible_sums[1]}  =  {sum} ({sum_mag})")
        max_sum_mag = sum_mag
print(f"Max: {max_sum_mag}")
