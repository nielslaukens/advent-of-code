...
for d in tree.dir_iter():
    path_size[d.path] = d.total_size()
sorted_path_size = []
for p in sorted(path_size.keys(), key=lambda k: path_size[k]):
    sorted_path_size.append((p, path_size[p]))

total_size = 70000000
needed = 30000000
free = total_size - tree.total_size()
to_free = needed - free
print(f"Need {to_free} more space")

for p, s in sorted_path_size:
    if s >= to_free:
        print(p, s)
        break
