...
head_pos = np.array([0, 0])  # x (horizontal, increases to the right), y (vertical, increases to the top)
tail_pos = np.array([0, 0])  # x (horizontal, increases to the right), y (vertical, increases to the top)
tail_positions = set()
for move in moves:
    for step in range(move.amount):
        head_pos = head_pos + move.direction.vector
