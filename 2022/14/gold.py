...
sand_source = (500, 0)
floor_y = cave.shape[1] + 1


def get_thing_at_c(coord: tuple[int, int]) -> str:
    global cave
    try:
        return cave[c]
    except IndexError:
        cave = numpy_tools.ndarray_auto_extending_assign(cave, c, '.', fill='.')
        return cave[c]
...
while True:
    ...
    for c in sand_next_candidates:
        if c[1] == floor_y:  # assume 'R'
            pass
        elif get_thing_at_c(c) == '.':
            sand_pos = c
            break
    else:  # come to rest
        cave[sand_pos] = 'o'
