...
def find_shortest_path_length(code: str, robot_nr: int) -> int:
    global robots
    #assert code.endswith('A')
    #print(code)
    if len(robots) - robot_nr == 0:
        return len(code)
    shortest_all = 0
    my_robot = robots[robot_nr]
    for digit in code:
        paths = my_robot.shortest_paths_to_key(digit)
        shortest_for_digit = None
        for path in paths:
            l = find_shortest_path_length(path + 'A', robot_nr + 1)
            if shortest_for_digit is None or l < shortest_for_digit:
                shortest_for_digit = l
        my_robot.pos = my_robot.inv_keymap[digit]
        shortest_all += shortest_for_digit
    #assert my_robot.key_under_pos == 'A'
    return shortest_all
...
for i, code in enumerate(codes):
    print(f"Scanning for code {i}: {code}")
    robots = [
        RobotArmPointingAtNumericKeypad(output=lambda d: None, name="robot at door"),
    ]
    for i in range(ROBOT_CHAIN_LEN):
        robots.append(RobotArmPointingAtRobotKeypad(output=lambda d: None, name=f"robot {i}"))
    robot3_input_len = find_shortest_path_length(code, 0)
    complexity = robot3_input_len * int(code[:-1])
    print(f"{robot3_input_len} * {int(code[:-1])} = {complexity}")
